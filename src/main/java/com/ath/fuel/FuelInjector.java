...
public final class FuelInjector {
    ...
    /**
     * not guaranteed to exist but much more likely than getActivity() -- in which case it will return the Application context
     */
    ...
    public static void ignite( Context context, Object instance ) {
        ...
        if ( isDebug() ) { // track ignite count
            if ( injector.igniteCount == null ) {
                injector.igniteCount = new WeakHashMap<Object, List<Exception>>();
            }

            List<Exception> exceptions = injector.igniteCount.get( instance );
            if ( exceptions == null ) {
                exceptions = new ArrayList<Exception>();
                injector.igniteCount.put( instance, exceptions );
            }
            exceptions.add( new Exception() );
            int count = exceptions.size();
            if ( count > 1 ) {
                FLog.e( "FUEL: Ignite-Count[%s] for %s", count, instance.getClass().getSimpleName() );
                for ( int i = 0; i < exceptions.size(); i++ ) {
                    Exception ex = exceptions.get( i );
                    StackTraceElement el = ex.getStackTrace()[1];
                    FLog.e( "FUEL: %s[%s] ignited from %s",
                            instance.getClass().getSimpleName(),
                            i,
                            el.getFileName() + ":" + el.getLineNumber()
                    );
                }
                return; // no need to multi-ignite
            }
        }
    ...
    public static final void initializeModule( FuelModule fuelModule ) {
        ...
        fuelModule.setActivityCallbacks( new ActivityLifecycleCallbacks() {
            ...
            public void onActivityCreated( Activity activity, Bundle savedInstanceState ) {
                FLog.leaveBreadCrumb( "onActivityCreate: activity = %s", activity );
    ...
    // FIXME: FUEL DOC for context singletons, the context is determined upon call of Lazy.get(), if you want to lock down the context you can
    // use FuelInjector.attain for non-lazy or Lazy.attain( Context )
    public static final <T> T attain( Context context, Class<T> type ) {
        return attain( context, type, CacheKey.DEFAULT_FLAVOR );
    }

    public static final <T> T attain( Context context, Class<T> type, Integer flavor ) {
        try {
     ...
     */
    @Nullable
    public static final <T> T findInstance( Context context, Class<T> type ) {
     ...
     */
    @Nullable
    public static final <T> T findInstance( Context context, Class<T> type, Integer flavor ) {
